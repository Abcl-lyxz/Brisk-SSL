...
### M1b RNG + AEAD
- [ ] `src/os/linux_rand.c`: getrandom syscall (per-arch numbers) -> poll /dev/random -> /dev/urandom, fail closed
- [ ] ChaCha20 + Poly1305 + AEAD (RFC 8439, Wycheproof)
...
- [ ] Vendor fiat curve25519_{32,64}, p256_{32,64} + vendor/VENDORED.md (commit, license) + NOTICE
