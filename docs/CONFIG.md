...
| `BRISK_ENABLE_QUIC` - QUIC v1 client transport (RFC 9000/9001, `src/quic/packet.c` + `src/quic/conn.c`): packets and header protection, CRYPTO frames on the one TLS 1.3 engine, transport parameters, CONNECTION_CLOSE. Internal API only until streams land (M6 item 2). | FULL | - (AES-GCM, ChaCha20-Poly1305, HKDF and the engine are always built) | 7915 (armv7hf) ... 14804 (mipsel); 9984 on x86_64 (FULL minus DEFAULT; i686 11621, aarch64 9863, armv5 11679, mips 14796, mips64 12616, riscv64 8252, ppc 12065). packet.c: x86_64 2877, i686 3119, aarch64 3034, armv7hf 2278, armv5 3630, mips 4368, mipsel 4360, mips64 3848, riscv64 2607, ppc 3446. RAM per connection: `sizeof(brisk__quic_conn)` (~3.7 KB: six key sets) + `brisk__quic_scratch_size()` (`BRISK_QUIC_CRYPTO_BUF` + 1/8 bitmap + 2 KB Initial + `BRISK__TLS13_OUT_MAX` Handshake send retention), besides the engine's own scratch |
