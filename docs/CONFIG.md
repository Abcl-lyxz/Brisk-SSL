...
| `BRISK_ENABLE_H2` - HTTP/2 client, `brisk_h2_*` (RFC 9113: `src/http/h2.c`) with HPACK (RFC 7541: `src/http/hpack.c` + `src/http/huffman.c`, the Huffman decoder and prefixed integer shared with QPACK). An optional module called explicitly over a connection whose ALPN is "h2"; never switched on by ALPN. | DEFAULT and FULL | - | h2 + hpack + huffman: 10012 (armv7hf) ... 18220 (mipsel); 13561 on x86_64 (size/baseline.json, M7). h2.c alone: x86_64 9097, i686 9529, aarch64 9304, armv7hf 6636, armv5 10182, mips 12320, mipsel 12372, mips64 12180, riscv64 7466, ppc 10428. hpack / huffman: x86_64 3698/743, i686 3471/775, aarch64 4003/719, armv7hf 2787/579, armv5 4207/819, mips 4920/840, mipsel 4948/840, mips64 4928/860, riscv64 3349/665, ppc 4335/791. RAM per connection (`brisk_h2_size()`, caller-owned): the `BRISK_H2_HEADER_TABLE_SIZE` ring (4 KB), two `BRISK_H2_MAX_HEADER_LIST` buffers (field block + scratch), 5 KB tx/rx, and per stream `BRISK_H2_STREAM_WINDOW` + `BRISK_H2_MAX_HEADER_LIST` |
| `BRISK_ENABLE_QUIC` - QUIC v1 client transport (RFC 9000/9001, `src/quic/packet.c` + `src/quic/conn.c`): packets and header protection, CRYPTO frames on the one TLS 1.3 engine, transport parameters, ACKs, loss recovery + integer NewReno (`src/quic/recovery.c`, RFC 9002), streams + flow control (`src/quic/stream.c`), connection IDs, CONNECTION_CLOSE. Public API `brisk_quic_*` (`src/quic/api.c`: sans-I/O, plus the blocking `brisk_quic_connect` in `src/os/linux_udp.c`). | FULL | - (AES-GCM, ChaCha20-Poly1305, HKDF and the engine are always built) | 7915 (armv7hf) ... 14804 (mipsel); 9984 on x86_64 (FULL minus DEFAULT; i686 11621, aarch64 9863, armv5 11679, mips 14796, mips64 12616, riscv64 8252, ppc 12065). packet.c: x86_64 2877, i686 3119, aarch64 3034, armv7hf 2278, armv5 3630, mips 4368, mipsel 4360, mips64 3848, riscv64 2607, ppc 3446. RAM per connection: `sizeof(brisk__quic_conn)` (about 8.4 KB on x86_64: six key sets, 32 sent-packet records, 3 x 8 received ranges, 8 stream slots, CIDs, the kept 1-RTT secrets; 3752 before M6 item 2) + `brisk__quic_scratch_size()` (`BRISK_QUIC_CRYPTO_BUF` + 1/8 bitmap + 2 KB Initial + `BRISK__TLS13_OUT_MAX` Handshake send retention + `BRISK_QUIC_MAX_STREAMS * (2 * BRISK_QUIC_STREAM_BUF + BRISK_QUIC_STREAM_BUF / 8)` stream rings, ~68 KB at the defaults), besides the engine's own scratch. `brisk__quic_scratch_size()` = 82432 B at the defaults (8 stream slots; 47616 B with 4) |
| `BRISK_ENABLE_H3` - HTTP/3 (RFC 9114, `src/http/h3.c`) with QPACK static table only (RFC 9204, `src/http/qpack.c`: our dynamic table capacity and blocked streams are 0). An optional module called explicitly over a blocking `brisk_quic` whose ALPN is "h3". Forces `BRISK_ENABLE_QUIC` and `BRISK_ENABLE_H2` on (QPACK reuses huffman.c and the shared field checks in h2.c; decision M7: no "H3 without H2" build). With cfg.alpn offering "h3", our QUIC transport parameters let the server open its 3 uni streams; other ALPNs are byte-identical. Needs `BRISK_QUIC_MAX_STREAMS >= 5`; concurrent requests = MAX_STREAMS - 4. | FULL | QUIC, H2 (forced) | h3 + qpack: 9201 (armv7hf) ... 16472 (mipsel); 11582 on x86_64 (h3 5428..10892, qpack 3773..5584 incl. the 99-entry static table). FULL also grows stream.c (local abort) and api.c (ALPN-gated TPs). DEFAULT +68..+684 B from the h2/h3 shared field and request checks and the Huffman count-only walk. RAM per connection (`brisk_h3_size()`, caller-owned): (2 + MAX_STREAMS - 4) x `BRISK_H3_MAX_HEADER_LIST` + the handle, ~49 KB at the defaults |
...
| `BRISK_H3_MAX_HEADER_LIST` | 8192 | 4096 to 65535 | Our HTTP/3 SETTINGS_MAX_FIELD_SECTION_SIZE (RFC 9114 4.2.2): a larger response header section aborts only that request (H3_EXCESSIVE_LOAD). It also bounds our encoded request header section. A **RAM** lever: `brisk_h3_size()` moves by (2 + MAX_STREAMS - 4) x the change. Only with `BRISK_ENABLE_H3`. |
| `BRISK_QUIC_CRYPTO_BUF` | 4096 | 4096 to 65536 | Out-of-order CRYPTO bytes a QUIC connection buffers at its current receive level. RFC 9000 7.5 makes 4096 the floor; data beyond the buffer during the handshake closes with CRYPTO_BUFFER_EXCEEDED. A **RAM** lever: `brisk__quic_scratch_size()` grows by the value plus 1/8 for the bitmap. Only with `BRISK_ENABLE_QUIC`; flash and the ABI are unaffected. |
| `BRISK_QUIC_MAX_STREAMS` | 8 | 2 to 64 (5+ with HTTP/3) | QUIC stream slots alive at once, ours and the server's (RFC 9000 2). Our `initial_max_streams_bidi + initial_max_streams_uni` may not exceed it (`brisk__quic_conn_init` refuses), and a slot freed by a server stream is what raises our MAX_STREAMS, so the server can never open more streams than there are slots. A **RAM** lever: each slot is `2 * BRISK_QUIC_STREAM_BUF + BRISK_QUIC_STREAM_BUF / 8` bytes of `brisk__quic_scratch_size()` plus 144 bytes in `brisk__quic_conn`. Only with `BRISK_ENABLE_QUIC`; flash and the ABI are unaffected. |
| `BRISK_QUIC_STREAM_BUF` | 4096 | 1024 to 65536, a multiple of 8 | The receive ring and the send ring of one QUIC stream, in bytes: the largest per-stream flow-control window we may advertise (`initial_max_stream_data_*` above it is refused, and every MAX_STREAM_DATA stays within consumed + the ring), so FLOW_CONTROL_ERROR is what keeps server data inside it. Also the most a stream buffers for sending before `brisk__quic_stream_write` returns a short count. RAM formula: `BRISK_QUIC_MAX_STREAMS * (2.125 * BRISK_QUIC_STREAM_BUF)` - 68 KB at the defaults, 8.5 KB with 4 x 1024 for small routers. |
